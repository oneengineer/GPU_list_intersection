...
//				show_cur_lists<<<1,1>>>(10,10);
				//getchar();
				move_indices<<<1,1>>>(len1,len2,block_size);
				//show_cur_lists<<<1,1>>>(len1,len2);
				algo2_search<<<D1,D2>>>(devV);
				//prefix_sum_oneCore<<<1,1>>>(devV, min(min(len1,len2),block_size) );
				//save_result<<<D1,D2>>>(devV);
			}
		}
		cudaDeviceSynchronize();
	...
	void test_cal_indices(){
		...
		while (true){
			...
			out(len1),outln(len2);
			if ( len1<=0 || len2 <= 0 ) break;
			if ( len1<=block_size || len2 <= block_2_size ) break;
			cal_indx<<<1,4>>>(block_size,block_2_size);
		}
