/*New feature:
	...
	pipeline ( cudpp unpipelin) is employed
*/
...
//------------------ prefix sum over ---------------
	...
	__global__ void algo2_search(int * V,int search_now){
		int id = CUID;
...
//		printf("  algo2 search id: %d  %d get [%d] %d\n",id, value,right,list2[right]);
	...
	void test_pipeline(){
		generate_case2();
		debug_a(list[0],n);outline;debug_a(list[1],m);//show debug
		int numStream;
		numStream = 2;
		cudaStream_t *streams = (cudaStream_t *) malloc(numStream * sizeof(cudaStream_t));
		for (int i = 0; i < numStream; i++)
		...
		int loops = 8;
		int devVinc = 0; //cycle of TWO for saving V
		int cal_pos = 0, search_pos = 0, save_pos  =0; // they are for L1, L2 position
		...
		while ( loops -- ){
			...
			devVinc = 1- devVinc;
			algo2_search<<<D1,D2,0,streams[1]>>>( devV[devVinc],search_pos );
			save_result<<<D1,D2,0,streams[0]>>>(saveV,save_pos);/*---- save part -----*/
			cu_host_print(saveV, block_size);	//debug
			move_result_addr<<<1,1,0,streams[0]>>>(saveV,save_pos);/*---- save part -----*/
			move_pos( cal_pos );move_pos( search_pos );move_pos( save_pos );
			outln(loops); // debug
		}
		cudaDeviceSynchronize();
